...
def build_rc5(rc5_sys, rc5_cmd):
    """Build RC5 message, return as int"""
    RC5_START = 0b100 + (0b010 * (rc5_cmd < 64))
    RC5_SYS = int(rc5_sys)
    RC5_CMD = int(rc5_cmd)
...
def wave_mnch(pi, DATA, PIN, PERIOD):
...
def send_command(pi, command_type, repeat=1):
    """Send a command, managing wave resources properly"""
    try:
        wave_id = wave_mnch(pi, build_rc5(CA_RC5_SYS, cmd[command_type]), PIN, RC5_PER)
...
def main():
    ...
    # Run cec-client and watch its output. No COM port is given, so cec-client
    # connects to the first device it detects (the kernel /dev/cecN adapter on
    # this system - the old hardcoded "RPI" VideoCore port name no longer
    # applies to the generic Linux CEC framework used on current Debian/RPi OS).
    p = subprocess.Popen(
    ...
    while p.poll() is None:
        ...
        if "TV (0): power status changed" in l:
            ...
            if final_state == "on" or final_state == "in transition from standby to on":
                ...
                send_command(pi, "ampon", repeat=4)
            ...
            # Turn amp off for standby state
                ...
                send_command(pi, "ampoff", repeat=4)
        ...
        # Handle any playback device power status changes
            ...
            p.stdin.flush()
            send_command(pi, "vol+", repeat=VOL_STEPS)
        elif VOL_DN in l:
            ...
            if mute_state:
                ...
                send_command(pi, "muteoff")
            else:
                # Currently unmuted, so mute
                send_command(pi, "muteon")
